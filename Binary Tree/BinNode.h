...
namespace DSLibrary
{
    ...
    struct BinNode {
        ...
        //构造函数
        BinNode(T e, BinNodePosi(T) p=NULL, BinNodePosi(T) lc=NULL, BinNodePosi(T) rc=NULL, int h=0, int np=0, RBColor c=RB_RED):
                data(e),parent(p),lc(lc),rc(rc),height(h),npl(np),color(c) {}
        int size() {}
        ...
        //比较器
        bool operator < (BinNode const & x) {return data<x.data;}
        bool operator > (BinNode const & x) {return data>x.data;}
        bool operator == (BinNode const & x) {return data==x.data;}
        bool operator != (BinNode const & x ) {return !(this==x);}
    };
}
#endif //DSL_BINNODE_H
